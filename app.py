...
with tab1:
    ...
    if st.session_state.history_state:
        ...
        # عكس ترتيب الأعمدة لجعلها من اليمين لليسار وتقليل العرض لتناسب الشاشة
        cols_order = df_history.columns.tolist()
        cols_order = cols_order[::-1]
        df_history_reversed = df_history[cols_order]

        st.dataframe(df_history_reversed, use_container_width=False, hide_index=True)
...
with tab4:
    ...
    if st.button("🔄 تحديث وتوليد التقرير التنفيذي الموحد", use_container_width=True, type="primary"):
        if not st.session_state.history_state:
            ...
            # توليد نتائج تحليلية مخصصة ومتباينة تفصيلية بحسب الدولة المتصدرة والبيانات المسجلة
            if top_score >= 80:
                dynamic_analytical_summary = f"""
                يقدم هذا التقرير تجميعاً تحليلياً متكاملاً لمخرجات أقسام المنصة الأربعة لدولة <b>{top_country}</b>، عاكساً رؤية استشرافية متقدمة لدعم سياسات الصناعات الثقافية والإبداعية العربية في عصر الذكاء الاصطناعي وفق منظور التفكير المنظومي:
                <br><br>
                <b>1. التشخيص القياسي والترتيب التراكمي (القسم الأول):</b> سجلت الدولة أداءً مرتفعاً ومستداماً بقيمة مركبة ({top_score_str})، مما يعكس تفوقاً هيكلياً في البنية التقنية ورأس المال البشري الممكن لتقنيات 'المبدع المعزز'، مع ضرورة استثمار هذه الريادة لقيادة التحالفات الإقليمية.
                <br><br>
                <b>2. استشراف السياسات وبدائل السيناريوهات (القسم الثاني):</b> بينت نتائج المحاكي أن السيناريو الطموح يضمن تعزيز السيادة الرقمية الكاملة وتصدير النماذج اللغوية الثقافية العربية المستقلة، مع الحفاظ على وتيرة الاستثمار الحالية لحماية الأمن الثقافي القومي.
                <br><br>
                <b>3. لوحة دعم القرار والتطعيم الثقافي (القسم الثالث):</b> توصي المنصة بإنشاء 'مجلس سيادي أعلى للذكاء الاصطناعي والثقافة' وتفعيل آليات التطعيم الثقافي وحوكمة استخدام النماذج التوليدية بكفاءة عالية.
                <br><br>
                <b>4. الخلاصة الاستراتيجية:</b> تمثل هذه المعطيات نموذجاً ريادياً متكاملاً يمكن تعميمه إقليمياً لتعظيم العائد التنموي للاقتصاد البرتقالي.
                """
            elif top_score >= 51:
                dynamic_analytical_summary = f"""
                يقدم هذا التقرير تجميعاً تحليلياً متكاملاً لمخرجات أقسام المنصة الأربعة لدولة <b>{top_country}</b>، عاكساً رؤية استشرافية متوازنة لدعم سياسات الصناعات الثقافية والإبداعية العربية في عصر الذكاء الاصطناعي وفق منظور التفكير المنظومي:
                <br><br>
                <b>1. التشخيص القياسي والترتيب التراكمي (القسم الأول):</b> حققت الدولة أداءً متوسطاً بقيمة مركبة ({top_score_str})، مما يشير إلى وجود اختناقات هيكلية في بعض سلاسل القيمة أو التشريعات التنظيمية تستوجب حزم تحفيزية وبرامج إعادة تأهيل (Upskilling).
                <br><br>
                <b>2. استشراف السياسات وبدائل السيناريوهات (القسم الثاني):</b> أظهرت نتائج المحاكي أهمية تبني السيناريو المعتدل والتوسع في التدريب المعتمد لتقليص فجوة الإحلال الخوارزمي ورفع كفاءة سوق العمل الإبداعي.
                <br><br>
                <b>3. لوحة دعم القرار والتطعيم الثقافي (القسم الثالث):</b> تركز التوصيات على تفعيل حاضنات الأعمال الإبداعية المشتركة وتطوير أطر البيئة التشريعية وحماية المصنفات الثقافية من القرصنة الرقمية.
                <br><br>
                <b>4. الخلاصة الاستراتيجية:</b> تتطلب المرحلة القادمة تدخلات استباقية وبرامج طوارئ استثمارية لردم الفجوات القائمة وتأمين تدفقات مالية داعمة للقطاع البرتقالي.
                """
            else:
                dynamic_analytical_summary = f"""
                يقدم هذا التقرير تجميعاً تحليلياً متكاملاً لمخرجات أقسام المنصة الأربعة لدولة <b>{top_country}</b>، عاكساً رؤية طارئة واستشرافية لدعم سياسات الصناعات الثقافية والإبداعية العربية في عصر الذكاء الاصطناعي وفق منظور التفكير المنظومي:
                <br><br>
                <b>1. التشخيص القياسي والترتيب التراكمي (القسم الأول):</b> عكست القراءة الحالية فجوة هيكلية حرجة بقيمة مركبة تبلغ ({top_score_str})، مما يستوجب تدخلاً فورياً معالجةً للاختناقات العميقة في البنية التحتية والتشريعات ورأس المال البشري.
                <br><br>
                <b>2. استشراف السياسات وبدائل السيناريوهات (القسم الثاني):</b> حذرت مخرجات المحاكي من استمرار الجمود الاستثماري، مؤكدة ضرورة تفعيل خطط الطوارئ الاستثمارية لحماية المهن الثقافية المعرضة للاندثار.
                <br><br>
                <b>3. لوحة دعم القرار والتطعيم الثقافي (القسم الثالث):</b> توجب التوصيات استدعاء إطار التكامل الوظيفي الإقليمي، دمج الاقتصاد غير الرسمي، وإطلاق برامج عاجلة للأمن الثقافي وحماية التراث الرقمي.
                <br><br>
                <b>4. الخلاصة الاستراتيجية:</b> وضع خارطة طريق إجرائية عاجلة لإنقاذ الأنشطة الإبداعية ورفع مساهمة الاقتصاد البرتقالي في التنمية المستدامة.
                """
            ...
            # تجهيز ملف الورد مع تضمين النتائج التحليلية المخصصة والمفصلة بالكامل بالتفصيل دون اختصار
            word_content = f"""
            <html dir="rtl">
            ...
            <body style="font-family: 'Cairo', Arial, sans-serif; text-align: right;">
                ...
                <hr>
                <h3>1. ريادة الأداء العام والإطار الإقليمي:</h3>
                ...
                <hr>
                <h3>2. ملخص النتائج التحليلية التراكمية والمعمقة لكافة أقسام المنصة:</h3>
                <p>{dynamic_analytical_summary}</p>
                <hr>
                <h3>3. إطار شجرة قرارات نقاط الرفع المنظومي (Meadows Leverage Points Framework):</h3>
                <ul>
                    <li><b>المعلمات والميزانيات (Parameters):</b> تعديل نسب الإنفاق المباشر وموازنات دعم التدريب.</li>
                    ...
                    <li><b>أهداف النظام (Goals):</b> توجيه السياسات الوطنية نحو تحقيق السيادة الرقمية وصون الأمن الثقافي.</li>
                    <li><b>النماذج الفكرية (Paradigms):</b> ترسيخ مفهوم الاقتصاد البرتقالي كركيزة أساسية للتنمية المستدامة.</li>
                </ul>
                <hr>
                <p style="text-align: center; font-weight: bold; color: #B45309; margin-top: 20px;">يُنصح بالمراجعة الدقيقة للقرارات الاستراتيجية</p>
                <p style="text-align: center; font-size: 12px; color: #64748B;">جميع الحقوق محفوظة للدراسة البحثية © 2026</p>
