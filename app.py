...
def get_region_and_features(country_name):
    """تصنيف الدول حسب الأبعاد الجغرافية، الإقليمية، التاريخية، والديموغرافية بأسلوب انسيابي."""
    ...
    if country_name in gulf_countries:
        return f"تنتمي دولة ({country_name}) إلى إقليم الخليج العربي؛ حيث تتسم بكتلة مالية استثمارية واعدة، وسرعات بنية تحتية رقمية فائقة، وتركيز استراتيجي عميق على التحول الذكي وقيادة الابتكار الإقليمي."
    elif country_name in nile_valley:
        return f"تنتمي دولة ({country_name}) إلى إقليم وادي النيل العريق؛ وتتميز بعمق تاريخي وحضاري فريد، وثقل ديموغرافي بشري كبير، ورأس مال أكاديمي وثقافي يمثل ركيزة صلبة للاقتصاد البرتقالي."
    elif country_name in north_africa:
        return f"تنتمي دولة ({country_name}) إلى إقليم شمال إفريقيا؛ وتتفرّد بتنوع ثقافي ولغوي متوسطي وإفريقي غني، وجسور تواصل إبداعي عابرة للحدود تعزز الانفتاح والتفاعل الرقمي المستدام."
    elif country_name in levant:
        return f"تنتمي دولة ({country_name}) إلى إقليم بلاد الشام؛ وتتميز بتراث إبداعي فكري أصيل، وششبكات مجتمعية نشطة، وكفاءات بشرية عالية التأهل وقادرة على تطويع الأدوات التقنية الحديثة."
    else:
        return f"تنتمي دولة ({country_name}) إلى نطاق العالم العربي الموسع؛ وتزخر بخصائص جيوستراتيجية وتاريخية فريدة داعمة للتكامل الإقليمي وتنوع الروافد الثقافية والإبداعية."
    ...
    with col_sim_out:
        ...
        if run_sim_btn:
            if sim_country_sel == "الدولة":
                ...
                # تحليل قياسي متمايز تماماً ومتغير حسب أرقام المحاور لكل دولة بدقة
                if scenario_dropdown.startswith("📉"):
                    simulated_val = max(0.0, base_val * 0.90)
                    analysis_text = f"تحذير هيكلي لـ ({sim_country_sel}): في ظل مسجل البنية التقنية عند ({t_val}%) ورأس المال البشري ({h_val}%)، فإن الجمود الاستثماري سيؤدي إلى تفاقم فجوة الإحلال الخوارزمي واندثار المهن الثقافية بنسبة تفوق 30%."
                    action_plan = f"التدخل الفوري لترميم البيئة التشريعية الضعيفة المسجلة عند ({r_val}%) وضخ تمويلات طارئة لحماية سلاسل القيمة المهددة."
                elif scenario_dropdown.startswith("📊"):
                    simulated_val = min(100.0, base_val * (1 + (combined_boost * 0.0022)))
                    analysis_text = f"تقدم ملحوظ لـ ({sim_country_sel}): بناءً على رصيد المحددات الثقافية ({c_val}%) وديناميكيات السوق البرتقالي ({e_val}%)، يتحقق نمو تدريجي في حوكمة الاستخدام التوليدي وتمكين مهارات 'المبدع المعزز'."
                    action_plan = f"التوسع في برامج إعادة التأهيل المستمر للاستفادة من الكوادر البشرية البالغة ({h_val}%) وتعزيز كفاءة الإنفاق الإبداعي."
                else:
                    simulated_val = min(100.0, base_val * (1 + (combined_boost * 0.0042)))
                    analysis_text = f"نجاح استراتيجي فائق لـ ({sim_country_sel}): توظيف البنية التحتية القوية ({t_val}%) والتشريعات المتقدمة ({r_val}%) لتحقيق السيادة الرقمية وتوطين النماذج الثقافية العربية."
                    action_plan = f"قيادة التحالفات التقنية الإقليمية، إطلاق المنصات السحابية الوطنية المفتوحة، وتوظيف المحددات الثقافية الراسخة ({c_val}%) لحراسة الأمن القومي الثقافي."
