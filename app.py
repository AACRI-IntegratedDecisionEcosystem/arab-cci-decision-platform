...
def get_region_and_features(country_name):
    """تصنيف الدول حسب الأبعاد الجغرافية، الإقليمية، التاريخية، والديموغرافية."""
    ...
    levant = ["المملكة الأردنية الهاشمية", "الجمهورية اللبنانية", "الجمهورية العربية السورية", "دولة فلسطينت", "دولة فلسطين"]
    
    if country_name in gulf_countries:
        return "إقليم الخليج العربي (شبه الجزيرة العربية) — تتميز بكتلة مالية استثمارية واعدة، سرعات بنية تحتية رقمية فائقة، وتركيز على التحول الذكي."
    elif country_name in nile_valley:
        return "إقليم وادي النيل — تتميز بعمق تاريخي وحضاري فريد، ثقل ديمو جغرافي بشري كبير، ورأس مال أكاديمي وثقافي عريق."
    elif country_name in north_africa:
        return "إقليم شمال إفريقيا — تتميز بتنوع ثقافي لغوي متوسطي وإفريقي، وجسور تواصل إبداعي عابرة للحدود."
    elif country_name in levant:
        return "إقليم بلاد الشام — تتميز بتراث إبداعي فكري غني، شبكات مجتمعية نشطة، وكفاءات بشرية عالية التأهل."
    else:
        return "نطاق العالم العربي الموسع — تتميز بخصائص جيوستراتيجية وتاريخية داعمة للتكامل الإقليمي والتنوع الثقافي."
    ...
    with col_sim_out:
        ...
        if run_sim_btn:
            if sim_country_sel == "الدولة":
                ...
                # خطة تحرك متمايز ومتغير تماماً حسب قيم أرقام المحاور والسيناريو لكل دولة
                if scenario_dropdown.startswith("📉"):
                    simulated_val = max(0.0, base_val * 0.90)
                    analysis_text = f"تحذير هيكلي لـ ({sim_country_sel}): في ظل مستويات البنية التقنية ({t_val}%) ورأس المال البشري ({h_val}%)، فإن الجمود الاستثماري سيؤدي إلى تآكل المكتسبات الرقمية."
                    if t_val < 50:
                        action_plan = f"خطة طوارئ عاجلة لترميم البنية التحتية المتقهقرة ({t_val}%) وضخ استباشرية في النطاق العريض، مع تعويض الفجوة في البيئة التنظيمية البالغة ({r_val}%)."
                    else:
                        action_plan = f"إعادة هيكلة موازنات دعم الكوادر البشرية لمعالجة القصور في المؤشر البشري ({h_val}%) وحماية الوظائف الإبداعية من الإحلال الخوارزمي."
                elif scenario_dropdown.startswith("📊"):
                    simulated_val = min(100.0, base_val * (1 + (combined_boost * 0.0022)))
                    analysis_text = f"تقدم ملحوظ لـ ({sim_country_sel}): استناداً إلى توازن المحددات الثقافية ({c_val}%) وديناميكيات السوق البرتقالي ({e_val}%)، يتحقق نمو تدريجي في حوكمة الاستخدام التوليدي."
                    if e_val >= 70:
                        action_plan = f"توجيه العوائد القوية للاقتصاد البرتقالي ({e_val}%) نحو توسيع برامج إعادة التأهيل للكوادر لتعظيم العائد من رأس المال البشري ({h_val}%)."
                    else:
                        action_plan = f"تحفيز تدفقات رأس المال المخاطر ورفع كفاءة البيئة التشريعية الحالية عند ({r_val}%) لدعم استدامة الأنشطة الإبداعية."
                else:
                    simulated_val = min(100.0, base_val * (1 + (combined_boost * 0.0042)))
                    analysis_text = f"نجاح استراتيجي فائق لـ ({sim_country_sel}): توظيف البنية التحتية العالية ({t_val}%) والتشريعات المتقدمة ({r_val}%) لتحقيق السيادة الرقمية الكاملة وتوطين النماذج الثقافية."
                    if c_val >= 80:
                        action_plan = f"قيادة التحالفات التقنية الإقليمية، إطلاق المنصات السحابية الوطنية المفتوحة، وتوظيف المحددات الثقافية الراسخة ({c_val}%) لتصدير المحتوى الرقمي العربي."
                    else:
                        action_plan = f"تكثيف الأرشفة الثلاثية للتراث الوطني لرفع سقف المحددات الثقافية ({c_val}%) بالتوازي مع تفعيل آليات الوسم المائي وحماية الملكية الفكرية."
        ...
        if generate_decision_btn:
            if not st.session_state.history_state or decision_country_sel == "الدولة":
                ...
                if country_data:
                    ...
                    # توصيات مخصصة بالكامل لكل دولة على حدة بناءً على درجات ومحاور الدولة
                    if score >= 80:
                        recs_list = [
...
<div style="background: {CBE_BG}; padding: 16px; border-radius: 8px; border-right: 4px solid {CBE_ORANGE_MID}; margin-top: 10px;" dir="rtl">
    <p style="font-weight: bold; color: {CBE_NAVY}; margin-bottom: 8px;">💡 إضافات تحليلية استراتيجية عميقة ومخصصة لدولة ({decision_country_sel}):</p>
    <ul style="margin: 0; padding-right: 20px; color: #1E293B; line-height: 1.8; text-align: right;">
    ...
    if st.button("🔄 تحديث وتوليد التقرير التنفيذي الموحد", use_container_width=True, type="primary"):
        if not st.session_state.history_state:
            ...
            <div style="background: {CBE_BG}; padding: 18px; border-radius: 10px; border-right: 5px solid {CBE_ORANGE_MID}; margin-bottom: 25px;" dir="rtl">
                ...
                <p style="margin-bottom: 8px;"><b>الخصائص الإقليمية والأبعاد الجغرافية:</b> {region_info}</p>
