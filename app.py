...
with tab4:
    ...
    if st.button("🔄 تحديث وتوليد التقرير التنفيذي الموحد", use_container_width=True, type="primary"):
        if not st.session_state.history_state:
            ...
            # تجهيز محتوى ملف الـ Word ليشمل الأقسام الأربعة المطلوبة بدقة
            countries_rows_html = ""
            for idx, row in df_rep.iterrows():
                countries_rows_html += f"<li><b>المركز ({idx + 1}): {row['الدولة']}</b> — المؤشر المركب: {row['المؤشر المركب (AACRI)']} % | البنية التقنية: {row['البنية التقنية (20%)']}% | رأس المال البشري: {row['رأس المال البشري (30%)']}%<br><i>التقييم: {row['التقييم المنظومي']}</i></li><br>"
            ...
            <body style="font-family: 'Cairo', Arial, sans-serif; text-align: right; line-height: 1.8;">
                ...
                <p style="text-align: center; color: #64748B;">ملخص استراتيجي موجه للقيادات العليا وصناع القرار - إطار مؤشر الجاهزية الذكية المركب (AACRI)</p>
                <hr>

                <h3 style="color: #78350F;">🏆 مؤشرات الأداء العام والريادة الإقليمية (التصنيف الجغرافي والديموغرافي والتاريخي):</h3>
                ...
                <h3 style="color: #78350F;">📋 ترتيب الملخص التراكمي للدول:</h3>
                <ul>
                    {countries_rows_html}
                </ul>
                <hr>

                <h3 style="color: #78350F;">📈 ملخص النتائج التحليلية التراكمية والمعمقة لكافة أقسام المنصة:</h3>
                ...
                <p><b>2. استشراف السياسات وبدائل السيناريوهات (القسم الثاني):</b> بينت مخرجات المحاكي أن التحفيز الموجه نحو برامج إعادة التأهيل (Reskilling) وتطوير البنية التحتية يرفع بفاعلية من القيمة المركبة للمؤشر ويقي الاقتصادات صدمات البطالة التكنولوجية.</p>
                <p><b>3. لوحة دعم القرار والتطعيم الثقافي (القسم الثالث):</b> عززت التوصيات المخصصة والديناميكية ضرورة إرساء مجالس عليا للسيادة الرقمية وتفعيل حاضنات الأنشطة الإبداعية المشتركة بما يصون الأمن الثقافي القومي.</p>
                <p><b>4. الخلاصة الاستراتيجية:</b> إن تضافر هذه المخرجات يضع بين أيدي صانعي القرار خارطة طريق إجرائية لترسيخ ركائز الاقتصاد البرتقالي وتعظيم العائد التنموي.</p>
                <hr>

                <h3 style="color: #78350F;">🌳 إطار شجرة قرارات نقاط الرفع المنظومي (Meadows Leverage Points Framework):</h3>
                <ul>
                    <li><b>1. المعلمات والميزانيات (Parameters):</b> تعديل نسب الإنفاق المباشر وموازنات دعم التدريب وإعادة التأهيل.</li>
                    <li><b>2. تدفق المعلومات (Information Flows):</b> توفير قواعد بيانات مرجعية ومنصات حصر رقمية ومؤشرات قياس آنية.</li>
                    <li><b>3. القواعد والحوكمة (Rules):</b> تشريع أطر الملكية الفكرية المشتركة ومعايير الوسم المائي لحماية الحقوق.</li>
                ...
                <hr>
                <p style="text-align: center; font-weight: bold; margin-top: 40px; color: #64748B;">جميع الحقوق محفوظة للدراسة البحثية © 2026</p>
            </body>
            </html>
            """
            
            st.download_button(
                label="📥 تحميل التقرير التنفيذي الشامل في ملف مستند (Word / .doc)",
